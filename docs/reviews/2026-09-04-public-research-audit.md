...
| Section tests mixed a fixed coordinate threshold with implicit relative tolerances | All three now test plane residuals against a time/normal-velocity-scaled roundoff allowance | First Linux runs exposed the zero-offset failure; nonzero-offset checks become stricter. Production and research gates are unchanged. |
