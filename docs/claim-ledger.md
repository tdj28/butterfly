...
| CLM-012 | The hub drifts robustly as `b` varies from `0.2` to `2.0`. | Partial new support over a narrower range: EXP-021 shows coherent raster motion for `b in [0.1,0.3]`; EXP-022 corrects stable period-3 and period-5 cycles along moving representative paths in all eleven frames. This does not establish a hub center, transition surface, or behavior through `b=2`. | GPU atlas plus phase-conditioned shooting and Floquet diagnostics now cover two cross-`b` family candidates. | Continue the transition surface and saddle-node/period-doubling curves in `(a,b,c)` with pseudo-arclength methods; define and track a hub center, detect folds/topology changes, and extend prospectively toward `b=2`. |
