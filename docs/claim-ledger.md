...
| CLM-003 | That parameter pair is a homoclinic point, and is the unique such point on the stated transition segment. | Original mechanism now has a precise independently reproduced nearby candidate, while the printed coordinate and uniqueness remain unestablished. EXP-155 shows that the Hopf-born period-1 family is not itself the proposed homoclinic orbit. EXP-329--337 construct nonlinear stable targets, scan both printed-coordinate directions and three radii, reject all coarse residual hulls at winding zero, and sharpen the fixed-`c` mismatch to `0.000162262` near `a=0.18255`. Corrected single shooting in EXP-340 stalls at `0.000135120`. Failure-bound 16-arc DOP853 EXP-341 then crosses the prospective root gate at `2.66211e-9`. Independent 32-arc Radau EXP-342 passes all gates at `1.08861e-9` and reproduces `a=0.1826436082` within `3.13e-11`. This strongly supports a connection near the printed hub height, but the candidate is `0.00284361` above Jones's printed `a=0.1798`; shrinking-radius persistence is still required before qualifying existence. | Analytic eigenspaces; nonlinear stable branches by backward integration; gauge-aligned unstable departures; common-sphere inward events; signed tangent residuals; three radii; orthogonal parameter scans; hash-bound receipts; residual winding and return-time continuity audits; corrected single shooting; analytic-variational DOP853 and doubled-segmentation Radau multiple shooting. No shrinking-radius root sequence or uniqueness domain yet. | Reproduce the Radau root at radii `0.025` and `0.02` with prospective parameter-persistence gates; then continue every root over a declared segment before making a bounded uniqueness statement. |
