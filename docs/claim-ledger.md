...
| CLM-003 | That parameter pair is a homoclinic point, and is the unique such point on the stated transition segment. | Original claim; now has a precise but unqualified nearby root nomination. EXP-155 shows that the Hopf-born period-1 family is not itself the proposed homoclinic orbit. EXP-329/331 find no stable-aligned return at the rounded coordinate. EXP-332--337 construct nonlinear stable targets, scan both printed-coordinate directions and three radii, reject all coarse residual hulls at winding zero, and sharpen the fixed-`c` mismatch to `0.000162262` near `a=0.18255`. Corrected single shooting in EXP-340 stalls at `0.000135120`. Failure-bound 16-arc EXP-341 then crosses the prospective root gate with maximum matching defect `2.66211e-9` at `a=0.1826436081`; its sole failed check is optimizer termination after it continued to the full budget. The huge direct-replay divergence is consistent with the measured `1.51e10` segmented Jacobian condition ratio and motivates independent boundary-value validation, not rejection. Thus Jones's approximate coordinate is plausibly close to a homoclinic connection, but existence, printed-coordinate identity, and uniqueness are not yet qualified. | Analytic eigenspaces; nonlinear stable branches by backward integration; gauge-aligned unstable departures; common-sphere inward events; signed tangent residuals; three radii; orthogonal parameter scans; hash-bound receipts; residual winding and return-time continuity audits; corrected single shooting; analytic-variational 16-arc multiple shooting. No independently reproduced or shrinking-radius root and no uniqueness domain yet. | Split the matched EXP-341 arcs into an independent 32-arc Radau correction; reproduce the root over shrinking spheres; then continue every root over a declared segment before making a bounded uniqueness statement. |
