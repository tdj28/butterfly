...
| CLM-012 | The hub drifts robustly as `b` varies from `0.2` to `2.0`. | Partial new support over a narrower range: EXP-021 shows coherent raster motion for `b in [0.1,0.3]`; EXP-022 corrects stable period-3 and period-5 cycles along moving representative paths in all eleven frames. EXP-023/024 refine three period-doubling seeds. EXP-025 through EXP-027 resolve a smooth period-5 `+1` crossing near `b=0.272283` with no `b` turn, rejecting a saddle-node interpretation for that traced branch while leaving the interacting branch unclassified. This still does not establish a hub center, transition surface, or behavior through `b=2`. | GPU atlas, phase-conditioned shooting, Floquet diagnostics, natural continuation, and fold-safe pseudo-arclength now cover two cross-`b` family candidates and four local stability events. | Solve the coupled `+1` eigencondition and second branch; continue transition and period-doubling surfaces in `(a,b,c)`; define and track a hub center; extend prospectively toward `b=2`. |
