...
| CLM-003 | That parameter pair is a homoclinic point, and is the unique such point on the stated transition segment. | Original claim; unestablished. EXP-155 shows that the Hopf-born period-1 family is not itself the proposed homoclinic orbit. EXP-329/331 find no stable-aligned return at the rounded coordinate. EXP-332/333 then construct both nonlinear stable-manifold targets and scan angle plus `c`: the mismatch falls coherently above the printed height and produces 25 direct radius-`0.02` near matches, but EXP-334 rejects all three coarse componentwise cells at winding number zero. Finer radius-`0.025` EXP-335 clears 39.14% return coverage and produces 141 direct near matches with minimum chord `0.00129410` near `c=10.3189`, yet all four residual hulls again have degree zero. Orthogonal EXP-336/337 sharpen the fixed-`c` mismatch to `0.000162262` near `a=0.18255`. Corrected single shooting in EXP-340 reduces it to `0.000135120` but stalls with a scaled Jacobian condition ratio near `1.47e5`. Thus a repeatable nearby near miss and a long-shooting representation failure are established, not an intersection; the approximate printed coordinates and unresolved boundary-value problem prevent either confirmation or rejection. | Analytic eigenspaces; nonlinear stable branches by backward integration; gauge-aligned unstable departures; common-sphere inward events; signed tangent residuals; three radii; orthogonal parameter scans; hash-bound receipts; residual winding and return-time continuity audits; corrected finite-difference single shooting. No nonzero-degree cell, solved manifold intersection, shrinking-radius root, independent-solver root, or uniqueness domain yet. | Execute failure-bound segmented multiple shooting from EXP-340's interior endpoint; reproduce any root over shrinking spheres and an independent integrator; then continue every root over a declared segment before making a bounded uniqueness statement. |
