...
Latest CLM-003 checkpoint: forty-five additional receipt-bound points,
EXP-418--462, raise the qualified curve total from 25 to 70 and the
pseudo-arclength total from 22 to 67.  Two adaptive doubled-step regimes and
their defect-aware reductions pass; thirty subsequent conservative steps
reach `(a,c)=(0.1798474905198,10.3169893885338)` with `3.19984e-9` maximum
defect and `8.23728e-10` minimum singular value.  The outgoing arm is now
reproduced under 53 newly computed tangents after EXP-408.  EXP-403 remains the
first sampled local `a` minimum and the closest point to `a=0.1798`; therefore
the printed intersection, any later return, global nonintersection, proof, and
uniqueness remain open.  This paragraph supersedes the earlier EXP-417 count retained in
...
| CLM-003 | That parameter pair is a homoclinic point, and is the unique such point on the stated transition segment. | The original mechanism is strongly numerically qualified at a revised coordinate; the printed coordinate and uniqueness are not. Independent 16-arc DOP853 and 32-arc Radau roots plus three-radius persistence place the fixed-`c=10.3084` connection near `a=0.182643608174`, `0.00284361` above Jones's printed `a`. Natural continuation and eight qualified pseudo-arclength points reach `(a,c)=(0.1798174978857,10.3170814887419)`, only `1.74979e-5` above the historical fixed-`a` section, with stable pre-fold projections near `c=10.31713524`. EXP-369--377 preserve wrong-side, boundary-wall, projected-conditioning, stationary trust-region, and non-descent Newton failures. EXP-382 rejects standard unconstrained collocation even as a zero-step positive control. EXP-383 then passes a bounded 512-arc weighted-plane control, halves the defect to `5.10888e-9`, and lifts the smallest measured singular value by `6.63x`. EXP-384 aborts before solving on incompatible initialization bounds; unwalled EXP-385 converges below both residual gates but returns to a backward, above-section intersection. Forward-predictor EXP-386 reaches its lower `c` wall at a `6.19e-7` matching floor; smaller-step EXP-387 improves the floor to `6.13e-8` but again lands on the prospective forward wall. Lower-nuisance-weight EXP-388 passes the zero-step root and conditioning controls, but its licensed EXP-389 step still hits the forward wall at `5.82e-8`. Physical-only predictor EXP-390 closes all residual gates yet remains on that wall, isolating the long-secant closing geometry. EXP-391 passes a Jacobian-derived local-tangent zero-step control; first-step EXP-392 retains tangent/conditioning but reaches forward-`c` and angle walls. EXP-393 removes the angle wall but still exhausts its budget on the forward-`c` wall. Quarter-step EXP-394 closes below the block-defect gate with passing conditioning but exposes an acceptance-incompatible forward wall. Wall-free EXP-395 finds an interior root but returns `7.03e-8` backward in `c` and narrowly misses conditioning. EXP-396 then passes the standard local-tangent-normal zero-step control with `4.01e-9` maximum defect and `1.69e-9` minimum singular value. EXP-397's forward replay remains fully interior and conditioned but returns `1.17e-7` backward in `c`. Recomputing at corrected EXP-396 in EXP-398 still returns `5.47e-8` backward. Fourfold-smaller EXP-399 then passes every prospective gate and adds a twelfth qualified point at `(a,c)=(0.1798174938833,10.3170815075378)`. EXP-400 reveals that holding `Delta c` fixed can reinflate normalized arclength as the local tangent rotates; EXP-401 removes that inflation, while EXP-402's quarter-size replay again returns backward in `c` but moves toward the historical fixed-`a` section. EXP-403 then passes every decreasing-`a`, unconstrained-`c` gate at `(a,c)=(0.1798174935369,10.3170815025596)`, adding a thirteenth qualified point and demonstrating numerical continuation through the local `c` fold. EXP-404 recomputes the tangent there and returns a clean full-state-forward root while both displayed parameters reverse, showing that neither coordinate is a valid local orientation gate at this scale. EXP-405 passes a secant-aligned signed-arclength replay at `(a,c)=(0.1798174936753,10.3170815017034)`, adding a fourteenth qualified point. Chained EXP-406/407/408 then pass at increasing step sizes and resolve EXP-403 as the first sampled local `a` minimum. EXP-409 reaches a clean farther outgoing root but fails only the fixed `5e-10` singular-value floor. With every gate unchanged, half-step EXP-410 passes in two evaluations at `(a,c)=(0.1798177050799,10.3170808940738)`, with `3.20048e-9` maximum defect and `1.24926e-9` minimum singular value, adding the eighteenth qualified point. EXP-411--414 recompute the tangent at each new root and pass four consecutive identical-gate half-steps in two evaluations apiece. EXP-414 reaches `(a,c)=(0.1798184775390,10.3170786400251)` but uses `78.5%` of the root-defect gate. Prospectively halved EXP-415 restores the defect margin, and EXP-416/417 repeat that smaller step successfully. EXP-417 reaches `(a,c)=(0.1798189322263,10.3170771259104)` with `3.20023e-9` maximum defect and `1.16598e-9` minimum singular value, adding the twenty-fifth qualified point. The first observed local turn stays above and moves away from the historical section, and its outgoing arm is reproduced under eight newly computed tangents after EXP-408. The receipt-bound manuscript figure exposes the minimum, defect-aware outgoing branch, and every preserved failure. The printed point, any later intersection, global nonintersection, and uniqueness remain open. | Analytic eigenspaces; nonlinear stable branches; common-sphere matching; corrected single and multiple shooting; DOP853/Radau and 16/32-arc agreement; three-radius persistence; two qualified natural steps; twenty-two qualified pseudo-arclength points through 512 arcs; preserved wrong-side and conditioning failures; passed bounded weighted-plane and local-tangent controls with source-centered guards and prospective tangent/conditioning floors. No historical-path intersection, computer-assisted proof, or uniqueness domain yet. | Continue the outgoing branch with adaptive conditioning- and defect-aware signed arclength; test for later turns or branches; separately define a bounded uniqueness/nonintersection audit. |
