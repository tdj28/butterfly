...
Last updated: 2026-09-10 (through EXP-519 fresh-response contact; not a new audit of every row)
...
and seven record only eight of the required nine returns before the fixed
horizon. No qualifying sampled fold bracket is found. The latter seven are
time-window-censored observations, not demonstrated domain holes or absent
returns. Resolve them with a bounded horizon extension and full prefix checks
before choosing event-sheet refinement or a new curve parameterization.
No continuous-u coverage/absence proof or Jones chain follows from the grid.
See the [complete result and figure](updates/2026-09-09-exp511-direct-curve-coverage.md).
...
The [public-repository audit](reviews/2026-09-04-public-research-audit.md)
supersedes incompatible interpretations in the historical narrative below:

- CLM-003 has 81 accepted numerical points through EXP-473 (78
  pseudo-arclength points). The manuscript figure retains 80 through EXP-472.
  The fine turn and its distance from the historical section have no
  parameter-error bound. Successive points in one formulation are correlated
  continuation samples, not independent confirmations or an existence proof.
- CLM-008's full-flow Floquet-zero center criterion is invalid. Exact finite
  smooth-flow monodromy is invertible. The scalar-map critical membership
  question remains meaningful only with a defined representation; see FND-060.
- Evidence for an eighth subcritical birth needs a daughter-to-parent
  convergence audit near the event; a stable parent and a strongly unstable
  sampled child do not alone establish the local normal form.
- A three-monotone-branch scalar map with two extrema is **bimodal**.
  Historical uses of "trimodal" for this object are terminology errors.
...
**EXP-489 update:** both selected left-region witnesses pass a direct
fixed-time section-grazing test. All 26 target integrations were retained;
both solvers reproduce pair birth/death on the predicted displacement side,
four prior accepted returns and square-root separation scaling. This identifies
...
| CLM-003 | That parameter pair is a homoclinic point, and is the unique such point on the stated transition segment. | Nearby numerical candidate supported; printed coordinate, uniqueness, and fine turn location remain open. Two integrators, 16/32 shooting arcs, and three matching radii place a fixed-c=10.3084 candidate near a=0.182643608174. EXP-473 brings the same continuation chain to 81 accepted points. Its sampled a minimum lies about 1.75e-5 above the historical section, but the ill-conditioned discretization has no parameter-error bound; this does not establish a physical fold or nonintersection. Finding a different candidate does not exclude another connection at the printed coordinate. | Analytic eigenspaces, stable/unstable manifold matching, multiple shooting, radius/solver checks at the initial root, and a receipt-bound continuation chain; failures retained. The figure contains the EXP-472 80-point snapshot. See the 2026-09-04 audit for the revised interpretation. | Independently reproduce the boundary-value solution; test time/radius/mesh/gauge convergence and bound parameter error before continuing or interpreting the tiny turn. A separate bounded search is needed for uniqueness/nonintersection. |
