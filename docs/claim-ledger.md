...
| CLM-003 | That parameter pair is a homoclinic point, and is the unique such point on the stated transition segment. | The original mechanism is strongly numerically qualified at a revised coordinate; the printed coordinate and uniqueness are not. Independent 16-arc DOP853 and 32-arc Radau roots plus three-radius persistence place the fixed-`c=10.3084` connection near `a=0.182643608174`, `0.00284361` above Jones's printed `a`. Qualified natural-continuation points EXP-347 and EXP-350 have slopes `-0.325514` and `-0.325531`. Fixed-coordinate corrections then stall, but gauge-aligned two-parameter pseudo-arclength EXP-360--362 pass through `(a,c)=(0.1802255499680,10.3158280790748)` with defects below `8.49e-9`. The latest slope `-0.32554896` points toward `a=0.1798` near `c=10.317135256`. The earlier floor is therefore not an immediate branch termination, although a later fold and the historical-path intersection remain open. | Analytic eigenspaces; nonlinear stable branches; common-sphere matching; corrected single and multiple shooting; DOP853/Radau and 16/32-arc agreement; three-radius persistence; two qualified natural steps; three qualified chained 128-arc pseudo-arclength steps; preserved direct-intersection failures with source-centered node guards. No historical-path intersection, computer-assisted proof, or uniqueness domain yet. | Continue pseudo-arclength to bracket exact `a=0.1798`, qualify the section root, then audit all roots on a declared segment before any bounded uniqueness statement. |
