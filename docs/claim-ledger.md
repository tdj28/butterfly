...
| CLM-003 | That parameter pair is a homoclinic point, and is the unique such point on the stated transition segment. | The original mechanism is now strongly numerically qualified at a revised coordinate; the printed coordinate and uniqueness are not. EXP-329--337 construct nonlinear stable targets and sharpen the fixed-`c` near miss. Corrected single shooting EXP-340 diagnoses long-map ill-conditioning. A 16-arc DOP853 root in EXP-341 is independently reproduced by 32-arc Radau EXP-342 at defect `1.08861e-9`. After preserved nuisance-gauge failures, exact-node EXP-344 and EXP-346 pass at radii `0.025` and `0.02`, with defects `5.49708e-9` and `5.60724e-9`; `a` remains near `0.182643608174` to about `1e-12` across all three radii. Thus a homoclinic connection near the printed hub height is strongly supported, but at `c=10.3084` it lies `0.00284361` above Jones's printed `a=0.1798`. Qualified continuation points EXP-347 and EXP-350 trace a smooth local root curve with successive secant slopes `-0.325514` and `-0.325531`, prospectively intersecting `a=0.1798` near `c=10.3171354`. | Analytic eigenspaces; nonlinear stable branches; common-sphere matching; orthogonal scans; residual winding audits; corrected single shooting; analytic-variational DOP853 and Radau multiple shooting; 16/32-arc agreement; three-radius persistence; two qualified local continuation points. No direct historical-path intersection, computer-assisted proof, or uniqueness domain yet. | Directly solve the boundary-value problem at fixed `a=0.1798` near `c=10.3171354`, then audit all roots on a declared segment before a bounded uniqueness statement. |
