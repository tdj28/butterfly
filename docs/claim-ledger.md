...
| CLM-003 | That parameter pair is a homoclinic point, and is the unique such point on the stated transition segment. | Original mechanism now has a precise independently reproduced and first-step radius-persistent nearby candidate, while the printed coordinate and uniqueness remain unestablished. EXP-155 shows that the Hopf-born period-1 family is not itself the proposed homoclinic orbit. EXP-329--337 construct nonlinear stable targets, scan both printed-coordinate directions and three radii, reject all coarse residual hulls at winding zero, and sharpen the fixed-`c` mismatch to `0.000162262` near `a=0.18255`. Corrected single shooting in EXP-340 stalls at `0.000135120`. Failure-bound 16-arc DOP853 EXP-341 crosses the root gate at `2.66211e-9`; independent 32-arc Radau EXP-342 passes at `1.08861e-9` and reproduces `a=0.1826436082` within `3.13e-11`. After EXP-343 exposes a nuisance-gauge boundary, exact-node EXP-344 passes at radius `0.025` with defect `5.49708e-9` and preserves `a` within `1.30e-13`. This strongly supports a connection near the printed hub height, but the candidate is `0.00284361` above Jones's printed `a=0.1798`; one more smaller radius and continuation remain required. | Analytic eigenspaces; nonlinear stable branches by backward integration; gauge-aligned unstable departures; common-sphere inward events; signed tangent residuals; orthogonal parameter scans; hash-bound receipts; residual winding and return-time continuity audits; corrected single shooting; analytic-variational DOP853 and doubled-segmentation Radau multiple shooting; radius persistence from `0.03` to `0.025`. No radius-`0.02` root or uniqueness domain yet. | Reproduce the Radau root at radius `0.02` with a prospective parameter-persistence gate; then continue every root over a declared segment before making a bounded uniqueness statement. |
