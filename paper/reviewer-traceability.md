...
| Reinjection was described using an unclear phase-space branched-manifold picture | `jones2012topological` | “Topology and symbolic tests still required” | RVR-005 | Reframed as a return-map hypothesis; EXP-127 rejects faster capture for the `a=0.148` transient branch, and the complete recovered UPO census persists across the bracket, so the predictive test must use invariant-manifold connectivity of the genuinely three-branch saddle rather than orbit birth/death |
