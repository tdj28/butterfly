...
| Two/three extrema oscillation was promising but insufficiently demonstrated | `jones2012topological`, `barrio2012topological` | Results; “Topology and symbolic tests still required” | RVR-003 | Controls, `a=0.140`, `a=0.145`, `a=0.147`, and `a=0.150` pass prospectively; EXP-123/124 expose finite-time conditioning sensitivity at `a=0.148`; blind PIM then qualifies `a=0.148` as two and `a=0.1485` as three under the identical definition, narrowing the sampled bracket to `[0.148,0.1485]`; EXP-127 explains the conditioning split as delayed but bounded transient capture; a continued curve and reinjection mechanism remain open |
