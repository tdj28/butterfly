#!/usr/bin/env python3
"""Pseudo-arclength continuation of the Jones-path period-2 child."""
...
from continue_period2_c_to_flip import (
    _dominant_nontrivial,
    _half_period_closure,
    _winding,
    first_real_minus_one_bracket,
)


def _variables(row: dict) -> np.ndarray:
    return np.r_[row["initial_state"], row["period_time"], row["parameters"]["c"]]
...
def _diagnose(
    ...
    *,
    ...
    parameters = RosslerParameters(a=a, b=b, c=float(point[4]))
    monodromy = flow_monodromy(parameters, point[:3], float(point[3]), config=solver)
    neutral = monodromy.multipliers[
        int(np.argmin(np.abs(monodromy.multipliers - 1.0)))
    ]
    dominant = _dominant_nontrivial(monodromy)
    return {
        "parameters": {"a": a, "b": b, "c": float(point[4])},
        "initial_state": point[:3].tolist(),
        "period_time": float(point[3]),
        ...
        "half_period_closure": _half_period_closure(
            parameters, point[:3], float(point[3]), solver
        ),
...
def main() -> int:
    ...
    if manifest.get("schema") != "butterfly.period2-c-arclength-to-flip-manifest.v1":
        raise SystemExit("unsupported period-2 c pseudo-arclength manifest")
    ...
    output = {
        "schema": "butterfly.period2-c-arclength-to-flip-receipt.v1",
        ...
        "passed": passed,
        "scientific_scope": (
            "identity-safe pseudo-arclength period-2 continuation and first -1 "
            "bracket; not an exact event or switched period-4 child"
        ),
    }
