#!/usr/bin/env python3
"""Replay every new product and all four decisions, without new integrations."""
import argparse
import json
...
def audit(output,expected_sha):
    ...
    if (saved['experiment_id']!='EXP-517' or saved['status']!='completed' or saved['binding']!=b
    ...
    marker=run.ROOT/'artifacts/EXP-517/target-once.json'
    ...
    if (startup['passed'] is not True or startup['isolated'] is not True or startup['new_integrations']!=0
            ...
            or json.loads(startup['stdout'])!=dict(valid=True,new_integrations=0,candidates=4,maximum_target_ivps=128)):
    ...
    cross=run.model.cross_representation(rows,p['numerical']['scales'])
    if cross!=saved['cross_representation']:
        raise ValueError('all-four cross-representation decision differs')
    ...
    return dict(experiment_id='EXP-517',passed=True,protocol_compliant=True,source_commit=b['source_commit'],
        plan_sha256=sha256(run.PLAN),summary_sha256=expected_sha,inputs=run.INPUTS,rows=rows,
        controls=control_result,cross_representation=cross,target_ivps=calls,output_bytes_including_summary=size,
