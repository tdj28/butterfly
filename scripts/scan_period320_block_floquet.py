#!/usr/bin/env python3
"""Continue a segmented period-320 cycle across a predicted flip."""
...
def main() -> int:
    ...
    if manifest.get("schema") != "butterfly.period320-block-floquet-scan-manifest.v1":
        raise SystemExit("unsupported period-320 scan manifest")
    ...
    source_parameters = RosslerParameters(
        a=a, b=float(candidate["target_b"]), c=c
    )
            ...
            row.update(
                {
                    ...
                    "half_period_closure": half_closure(
                        nodes[0], duration, parameters, solver
                    ),
                    "half_node_rms": float(
                        np.sqrt(
                            np.mean(
            ...
            brackets.append(
                {
                    ...
                    "prediction_error": 0.5 * (left["b"] + right["b"])
                    - predicted_b,
    ...
    qualifying_brackets = [
        ...
        and abs(bracket["prediction_error"])
        <= acceptance["max_prediction_error"]
    ...
    output = {
        "schema": "butterfly.period320-block-floquet-scan.v1",
