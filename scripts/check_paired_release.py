...
def main():
    ...
    observed = gate["verify_pushed_source"](ROOT, args.source_commit, args.remote_ref)
    ...
    for path in bootstrap:
        raw = gate["committed_file"](ROOT, args.source_commit, path)
    ...
    tracked = gate["git"](ROOT, "ls-tree", "-r", "--name-only", args.source_commit).decode().splitlines()
    ...
    required = set(mapping.values()) | set(bootstrap) | tests | {
        ...
        "scripts/qualify_paired_sampling.py"}
    files = {}
    for path in sorted(required):
        raw = gate["committed_file"](ROOT, args.source_commit, path)
        files[path] = dict(bytes=len(raw), sha256=gate["digest"](raw))
    inventory_sha = gate["verify_inventory"](ROOT, args.source_commit, files, required_paths=required)
    if args.mode == "reviewed":
        result = gate["verify_reviewed_release"](ROOT, args.release, args.source_commit, args.remote_ref,
