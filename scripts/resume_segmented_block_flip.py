#!/usr/bin/env python3
"""Resume a segmented block-Floquet refinement from retained endpoints."""
...
def main() -> int:
    ...
    if manifest.get("schema") != "butterfly.segmented-block-flip-resume-manifest.v1":
        raise SystemExit("unsupported segmented resume manifest")
    ...
    if prior.get("schema") != manifest["prior_schema"] or prior.get("passed"):
        raise SystemExit("bound prior must be the failed refinement receipt")
    if prior["segment_count"] != manifest["segment_count"]:
        raise SystemExit("bound prior segment count mismatch")
    ...
    def evaluate(b, seed):
        ...
        return {
            ...
            "floquet": floquet,
            "multiplier_residual": floquet["dominant_nontrivial_multiplier"]["real"] + 1.0,
            "nodes": nodes.tolist(),
        }
    ...
    output = {
        "schema": "butterfly.segmented-block-flip-resume.v1",
