#!/usr/bin/env python3
"""One bounded all-five fold search, seeded by an observed midpoint return."""
import argparse
import json
...
from scripts import exp513_candidate_folds as model

ROOT=previous.ROOT
PLAN=ROOT/'experiments/manifests/EXP-513-candidate-fold-qualification.json'
RECEIPT='docs/experiments/receipts/EXP-512-censored-return-extension-result.json'
INPUTS=dict(previous.INPUTS,**{RECEIPT:'c89bb30f90bf26a4f4902e349459b234201a66b00d3d2b4a500d6849fdb96e76'})
EXPLICIT=['scripts/run_exp513_candidate_folds.py','scripts/audit_exp513_candidate_folds.py',
    'scripts/exp513_candidate_folds.py','scripts/verify_exp512_public_extension.py',
    'tests/test_exp513_candidate_folds.py','docs/experiments/EXP-513-candidate-fold-qualification.md',
    'experiments/manifests/EXP-513-candidate-fold-qualification.json']
...
def expected():
    return dict(experiment_id='EXP-513',status='prospective-outcome-informed-five-candidate-search',
        inputs=INPUTS,candidates=model.selection(inputs()),targets=coverage.load()['selection']['targets'],
        numerical=base.load()['fold'],solvers=['DOP853','Radau'],attempts=1,
        seed_rule='u midpoint; regular paired census; mean observed ninth-return time',
        ...
        source_paths=sorted(set(previous.load()['source_paths'])|set(EXPLICIT)))


def load():
    p=json.loads(PLAN.read_bytes())
    if p!=expected():
        raise ValueError('frozen all-five plan differs')
    return p
...
def startup(p):
    target=Path(tempfile.mkdtemp(prefix='exp513-startup-')).resolve()
    ...
    code=(f'import sys;from pathlib import Path;root=Path({str(target)!r});'
        ...
        'from scripts.run_exp513_candidate_folds import main;main();'
...
def execute(output,source,remote):
    ...
    def budget(integrating=False):
        ...
        if time.monotonic()-start>p['limits']['wall_seconds'] or shutil.disk_usage(ROOT).free<p['limits']['minimum_free_bytes']:
            raise RuntimeError('EXP-513 time/free-space cap')
        if integrating:
            if calls>=p['limits']['target_ivps']:
                raise RuntimeError('EXP-513 IVP cap')
            calls+=1
    def probe(c):
        folder=output/c['id']
        folder.mkdir()
    ...
    def shoot(c):
        ...
        return rows
    def timeout(*_):
        raise TimeoutError('EXP-513 wall deadline')
    ...
    try:
        ...
        budget()
        save('summary.json',dict(experiment_id='EXP-513',status='completed',binding=binding,rows=rows,
...
def main():
    from scripts import audit_exp513_candidate_folds  # noqa: F401
