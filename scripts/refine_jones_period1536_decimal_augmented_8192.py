#!/usr/bin/env python3
"""Extend the independent period-1536 augmented event to 8,192 steps."""
...
from audit_jones_period1536_decimal_augmented_independent import accepted_a_envelope
...
from refine_jones_period768_decimal_augmented import convergence_ratio, correct_profile
...
SCHEMA = "butterfly.jones-period1536-decimal-augmented-8192-manifest.v1"
...
def main() -> int:
    ...
    if manifest.get("schema") != SCHEMA:
        raise SystemExit("unsupported period-1536 8192-step manifest")
    ...
    lower_a, upper_a, successful_rows = accepted_a_envelope(continuation)
    envelope = manifest["continuation_envelope"]
    if successful_rows < int(envelope["minimum_successful_rows"]):
    ...
    with localcontext() as context:
        ...
        source_nodes = [
            [Decimal(value) for value in row] for row in independent["nodes_decimal"]
        ]
        source_tangents = [
        ...
        a_values = [Decimal(row["a_decimal"]) for row in previous_profiles] + [parameter]
        period_values = [
        ...
        previous_extrapolated_period = Decimal(independent["extrapolated_period_decimal"])
    ...
    checks = {
        ...
        <= Decimal(str(acceptance["maximum_successive_richardson_period_difference"])),
        "continuation_envelope": (
            lower_a <= parameter <= upper_a and lower_a <= extrapolated_a <= upper_a
        ),
        "source_neighborhood": (
