...
def family_summaries(data):
    summaries = []
    for depth,direction in ((4,0),(4,1),(8,0),(8,1)):
        for case,region in (("local-a025-c083",0),("local-a025-c083",1),("local-a027-c083",0),("local-a027-c083",1)):
            rows = [d for d in data if (d["case"],d["region"],d["depth"],d["direction"]) == (case,region,depth,direction)]
            if not rows:
                raise ValueError("missing family")
            summaries.append(dict(family_id=rows[0]["family_id"],
                screened_regular=sum(d["analysis"]["screened_regular"] for d in rows),candidates=len(rows),
                cuts={key:sum(not d["analysis"]["checks"][key] for d in rows) for key in ("point_numerics","point_projection","input_sign","time_coherence")},
                unavailable_solver_points=sum(s["xy"] is None for d in rows for s in d["samples"])))
    return summaries
...
def plot(source,anchor,output):
    ...
    markers = ["<","o",">"]
    families = family_summaries(data)
    for ri,(depth,direction) in enumerate(((4,0),(4,1),(8,0),(8,1))):
        for ci,(case,region) in enumerate((("local-a025-c083",0),("local-a025-c083",1),("local-a027-c083",0),("local-a027-c083",1))):
            ...
            family = families[ri*4+ci]
            checks,regular = family["cuts"],family["screened_regular"]
            ...
            if missing != family["unavailable_solver_points"]:
                raise ValueError("plotted exclusions differ")
    regular = sum(d["analysis"]["screened_regular"] for d in data)
    fig.suptitle("Screening the return geometry before assigning symbols",fontsize=20,y=.97)
    ...
    fig.text(.075,.064,"Cuts: N numerical pair, P numerics + input projection, S input-tangent sign, T event-time coherence. Flags overlap; they do not diagnose a grazing root.",fontsize=8.4)
    fig.text(.075,.043,"Black x: numerical pair fails. Black +: numerics pass, input projection fails. Candidate numbers are local to each family; panel axis ranges differ.",fontsize=8.4)
    ...
    plt.close(fig)
    receipt = dict(figure_id=FIGURE,title="Screening the return geometry before assigning symbols",
        ...
        bindings=[dict(path=p,sha256=sha256(ROOT/p)) for p in ("scripts/plot_exp491_event_sheets.py","scripts/audit_exp491_event_sheet_probe.py","scripts/run_exp491_event_sheet_probe.py","python/butterfly/event_sheet_probe.py",run.PLAN.relative_to(ROOT).as_posix(),"experiments/manifests/EXP-490-candidates.json","docs/experiments/receipts/EXP-490-direct-fold-result.json")],
        ...
        units="Dimensionless model state coordinates, with no additive tick offsets. Panel ranges differ and are not uncertainty intervals.",
...
def verify(output):
    ...
    if r["families"] != family_summaries(r["derived_values"]):
        raise ValueError("family aggregation differs")
