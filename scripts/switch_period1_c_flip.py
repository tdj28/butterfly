#!/usr/bin/env python3
"""Switch the fixed-(a,b) period-1 flip onto its period-2 child in c."""
...
def _parent_variables(row: dict) -> np.ndarray:
    return np.r_[
        row["initial_state"],
        2.0 * float(row["period_time"]),
        float(row["parameters"]["c"]),
    ]


def _parent_distance(point: np.ndarray, rows: list[dict]) -> float:
    ...
    parent = np.empty(5, dtype=float)
    for index in range(3):
        parent[index] = np.interp(
            point[4], c_values, [row["initial_state"][index] for row in ordered]
        )
    parent[3] = np.interp(
        point[4], c_values, [2.0 * row["period_time"] for row in ordered]
    )
    parent[4] = point[4]
    return float(np.linalg.norm(point - parent))


def _half_period_closure(row: dict, solver: SolverConfig) -> float:
...
def main() -> int:
    ...
    if manifest.get("schema") != "butterfly.period1-c-flip-switch-manifest.v1":
        raise SystemExit("unsupported period-1 c-flip switch manifest")
    ...
    if event_receipt.get("schema") != "butterfly.period1-c-flip-receipt.v1":
        raise SystemExit("event receipt is not the period-1 c flip")
    if parent_receipt.get("schema") != "butterfly.hopf-period1-to-hub-receipt.v1":
        raise SystemExit("parent receipt is not the Hopf-to-hub period-1 branch")
    ...
    output = {
        "schema": "butterfly.period1-c-flip-switch-receipt.v1",
    ...
    output["scientific_scope"] = (
        "local period-2 branch switch at the first fixed-path flip; independent "
