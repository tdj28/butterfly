#!/usr/bin/env python3
"""Refine a period-10 Floquet -1 event while enforcing half-period identity."""
...
def evaluate(b, rows, a, c, solver, corrector):
    ...
    multiplier = complex(
        max(np.delete(monodromy.multipliers, neutral), key=abs)
    )
...
def main():
    ...
    if left["multiplier_residual"] * right["multiplier_residual"] > 0.0:
        raise RuntimeError("period-10 flip is not bracketed")
    evaluations = []
    ...
    output = {
        "schema": "butterfly.period10-flip-refinement.v1",
